...
const signIfPresent = async (urlOrKey) => {
    ...
    // (tymczasowo) logi do debugowania:
    // console.log("SIGN input:", urlOrKey);
    // console.log("SIGN key:", key);

    // sanity check: key nigdy nie powinien być URL-em
...
const getHomeFacts = async (req, res) => {
    try {
            ...
            include: [
                {
                    model: Creator,
                    as: "creator",
                    required: false, // możesz dać true jeśli nie chcesz albumów bez aktywnego creatora
                    where: { isActive: true },
                    include: [
                        {
                            model: User,
        ...
        console.error(err);
        return res.status(500).json({ message: "Failed to load home facts" });
    }
};
